...
function CouponsCustomerActionbar(props) {
  ...
  const handlePurchaseCoupons = () => {
    if (props.values.checked.length === 0)
      setTimeout(() => {
        alert("Please select atleast one coupon to purchase.");
      }, 0);
    for (let index of Object.values(props.values.checked)) {
      const currentRef = props.couponsRef.current[index].current;
      const coupon =
        ...
        "\nDescription: " +
        currentRef.children[5].children[0].innerHTML +
        "\nPrice: " +
        currentRef.children[6].children[0].innerHTML +
        "\nCategory: " +
        currentRef.children[7].children[0].innerHTML +
        "\nRelease Date: " +
        currentRef.children[8].children[0].innerHTML +
        "\nExpiration Date: " +
        currentRef.children[9].children[0].innerHTML;
      CustomerService.purchaseCoupon(currentRef.children[0].id).then(
        () => {
          setTimeout(() => {
            alert(coupon + "\nCoupon purchase was successful.");
          }, 0);
            ...
            if (error.response.data.response) {
              if (error.response.data.errorCode === "CPN-002.011")
                setTimeout(() => {
                  alert(
                    coupon +
                      "\nCoupon purchase was unsuccessful, you already own the coupon."
                  );
                }, 0);
                ...
                setTimeout(() => {
                  alert(
                    coupon +
                      "\nCoupon purchase was unsuccessful, coupon is out of stock."
                  );
                }, 0);
              else {
                setTimeout(() => {
...
export default CouponsCustomerActionbar;
