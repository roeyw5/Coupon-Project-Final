...
function AdminCustomerActionbar(props) {
  ...
  const handleSearchId = () => {
    ...
    if (isMinimum(userOption, 1))
      AdminService.getCustomer(userOption).then(
        (result) => {
          props.setCustomers([result.data]);
          resetSelection();
        },
        (error) => {
          try {
            if (error.response.data.response) {
              setTimeout(() => {
                alert(
                  'No customer with the id of "' + userOption + '" was found.'
                );
              }, 0);
              return;
            }
  ...
  const handleUpdateCustomer = () => {
    if (props.values.checked.length === 0)
      setTimeout(() => {
        alert("Please select atleast one customer to update.");
      }, 0);
    else if (props.values.checked.length > 1)
      setTimeout(() => {
        alert("You can only update one customer at a time.");
      }, 0);
    else history.push("/admin/customers/update-customer");
  };
  ...
  return (
    <>
      ...
      <Switch>
        <Route path="/admin/customers" exact></Route>
...
export default AdminCustomerActionbar;
