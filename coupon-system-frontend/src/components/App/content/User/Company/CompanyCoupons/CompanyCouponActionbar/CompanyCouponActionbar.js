...
function CompanyCouponActionbar(props) {
            ...
            .getCoupons().then(
                ...
                error => {
                    try {
                        ...
                        if (error.response) {
                            setTimeout(() => { alert("Your session has expired, please login") }, 0)
                            AuthenticationService.logOut();
                        }
                    } catch {
                        setTimeout(() => { alert("Server not responding, try again later.") }, 0)
                        AuthenticationService.logOut();
                    }
                }
            )
            ...
            CompanyService
                .getCouponsPrice(userOption).then(
                    ...
                    error => {
                        try {
                            if (error.response.data.response) {
                                setTimeout(() => { alert("No coupons with the maximum price of \"" + userOption + " \"were found.") }, 0)
                                return
                            }
                            if (error.response) {
                                setTimeout(() => { alert("Your session has expired, please login") }, 0)
    ...
    const handleUpdateCoupon = () => {
        if (props.values.checked.length === 0)
            setTimeout(() => { alert("Please select atleast one coupon to update.") }, 0)
    ...
    const handleDeleteCoupon = () => {
        if (props.values.checked.length === 0)
            setTimeout(() => { alert("Please select atleast one coupon to delete.") }, 0)
        else
            for (let index of Object.values(props.values.checked)) {
                const currentRef = props.couponsRef.current[index].current
    ...
    return (
        <>
            <div className='CompanyCouponActionbar'>
                ...
                <button className='action-sidebar-button' title='All Coupons' onClick={handleAllCoupons}>Show All</button>
                <div className='action-sidebar-button-seperator'></div>
                <button className='action-sidebar-button' title='Filter Category' onClick={handleSearchCategoryPush}>Filter By Category</button>
                <div className='action-sidebar-button-seperator'></div>
                <button className='action-sidebar-button' title='Filter Price' onClick={handleSearchPrice}>Filter By Price</button>
                <div className='action-sidebar-button-seperator'></div>
...
export default CompanyCouponActionbar