...
class CustomerService {
  ...
  async getCouponsCategory(category) {
    return await axios.get(
      "http://localhost:8080/customer/get-coupons-category?category=" + category
    );
  }

  async getCouponsPrice(price) {
    return await axios.get(
      "http://localhost:8080/customer/get-coupons-max-price?maxPrice=" + price
    );
  }

  async purchaseCoupon(couponId) {
    return await axios.put(
      "http://localhost:8080/customer/purchase-coupon?couponId=" + couponId
    );
  }
